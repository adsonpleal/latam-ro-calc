...
export const BUFF_BONUS_LABELS: Record<string, string> = {
  ...
  ...Object.fromEntries(([
    ['water', 'Água'], ['wind', 'Vento'], ['earth', 'Terra'], ['fire', 'Fogo'], ['neutral', 'Neutro'],
  ] as const).flatMap(([key, name]) => [
    [`p_element_${key}`, `Dano físico contra ${name}`],
    [`m_element_${key}`, `Dano mágico contra ${name}`],
  ])),
  pAtk: 'P.ATQ', sMatk: 'S.ATQM', cRate: 'T.CRIT',
  ...
  venomImpression: 'Reduz Res. Veneno do alvo',
  ...
  fireInsigniaOnTarget: 'Dano de Água recebido', waterInsigniaOnTarget: 'Dano de Vento recebido',
  windInsigniaOnTarget: 'Dano de Terra recebido', earthInsigniaOnTarget: 'Dano de Fogo recebido',
  volcano: 'Dano de Fogo', deluge: 'Dano de Água', violentGale: 'Dano de Vento',
  insignia_ratio_fire: 'Dano mágico de Fogo', insignia_ratio_water: 'Dano mágico de Água',
  insignia_ratio_wind: 'Dano mágico de Vento', insignia_ratio_earth: 'Dano mágico de Terra',
  acd_magic_wind: 'Pós-conjuração (magias de Vento)', propertyAtk: 'Propriedade da arma',
  hp: 'HP máx.', sp: 'SP máx.', hpPercent: 'HP máx. %', flee: 'Esquiva', healReceived: 'Cura recebida %',
