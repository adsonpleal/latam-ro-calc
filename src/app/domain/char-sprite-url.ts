...
import { ClassIcon } from 'src/app/jobs/_class-name';
 ...
 *  - omitting `canvas` entirely makes the gateway crop tight to the drawn pixels
 *    (measured: 37x70 bare Animista up to 99x143 with three headgears and a cape),
 *    which is what `tightCrop` asks for.
 *
...
/** Options for callers that don't want the shared fixed canvas. */
export interface CharSpriteOptions {
  /**
   * Drop the `canvas` parameter so the gateway crops to the drawn pixels instead of
   * padding into a fixed frame. Nothing can be clipped, but the returned size varies
   * per build, so the caller has to read it back and lay out accordingly. The social
   * card does; the saved-sim list wants every figure on the same baseline and keeps
   * the fixed canvas.
   */
  tightCrop?: boolean;
}
...
export const buildCharSpriteUrl = (
  ...
  if (!options.tightCrop) p.set('canvas', CANVAS);
