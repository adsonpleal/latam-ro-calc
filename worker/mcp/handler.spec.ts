...
describe('health', () => {
  it('reports the loaded dataset', async () => {
    ...
    // 17218 is the union of the two item sets: the 11280 distinct ids in items-core plus
    ...
    // nothing on the way.
    await expect(res.json()).resolves.toMatchObject({ ok: true, items: 17218, classes: 40 });
  });
});
